...
export default function Blog({
  ...
  const youtubeId = isYoutube && 'id' in image
    ? (image as { id?: string }).id || getYoutubeIdFromUrl(cleanUrl)
    : '';
        ...
        {/* <div className="mb-3">
          <span className="inline-block rounded-full bg-blue-600/20 px-3 py-1 text-xs font-medium text-blue-400 border border-blue-500/30">
            {action}
                 ...
                 <div className="p-6">
           <h2 className="mb-4 text-xl font-bold text-gray-900 dark:text-white transition-colors duration-200 group-hover:text-blue-600 dark:group-hover:text-blue-300 lg:text-2xl line-clamp-2 h-16 flex items-start">{title}</h2>
