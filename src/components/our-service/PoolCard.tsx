...
export default function PoolCard({
  ...
  const renderNum = (val: number | null) =>
    loading ? "…" : <CountUp end={(val ?? 0) as number} duration={1} separator="," />;

  const renderAda = (str: string | null) => {
    if (loading) return "…";
    if (!str) return "-";
      ...
      <div className="relative p-6">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-lg bg-gray-50 dark:bg-gray-900/40 p-3 border border-gray-200/60 dark:border-gray-700/60">
            ...
            <div className="mt-1 text-base font-semibold text-gray-900 dark:text-white">{renderNum(delegators)}</div>
          </div>
          <div className="rounded-lg bg-gray-50 dark:bg-gray-900/40 p-3 border border-gray-200/60 dark:border-gray-700/60">
            <div className="text-[12px] uppercase tracking-wide text-gray-500 dark:text-gray-400">Blocks</div>
            <div className="mt-1 text-base font-semibold text-gray-900 dark:text-white">{renderNum(blocks)}</div>
          </div>
          <div className="rounded-lg bg-gray-50 dark:bg-gray-900/40 p-3 border border-gray-200/60 dark:border-gray-700/60 min-w-0">
            ...
            <div className="mt-1 text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{renderAda(stake)}</div>
          </div>
          <div className="rounded-lg bg-gray-50 dark:bg-gray-900/40 p-3 border border-gray-200/60 dark:border-gray-700/60 min-w-0">
            <div className="text-[12px] uppercase tracking-wide text-gray-500 dark:text-gray-400">Pledge</div>
            <div className="mt-1 text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{renderAda(pledge)}</div>
          </div>
        </div>
        <div className="mt-5">
          <button
